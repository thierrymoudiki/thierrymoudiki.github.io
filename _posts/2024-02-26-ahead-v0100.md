...
Where $$\hat{f}_{-i}\left(\mathbf{z}_i\right)$$ is a model statistical learning model $$f$$'s prediction without the $$i$$-th observation in the training set. $$\mathbf{z}_i$$ is the $$i$$-th observation, and $$\mathbf{S}_{i i}$$ is the $$i$$-th diagonal element of the hat matrix (a.k.a smoother, a matrix so that $$\hat{y} = \mathbf{S} y$$). **LOOCV is [valid for time series](https://robjhyndman.com/papers/cv-wp.pdf) data**. **Keep in mind**, however, that [time series cross-vaidation](https://thierrymoudiki.github.io/blog/2020/03/27/r/misc/crossval-2) will give different results. 

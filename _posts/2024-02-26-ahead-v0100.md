...
Where $$\hat{f}_{-i}\left(\mathbf{z}_i\right)$$ is a model statistical learning model $f$ fitted without the $$i$$-th observation, $$\mathbf{z}_i$$ is the $$i$$-th observation, and $$\mathbf{S}_{i i}$$ is the $i$-th diagonal element of the hat matrix (a.k.a smoother, a matrix so that $$\hat{y} = \mathbf{S} y$$).

**LOOCV is [valid for time series](https://robjhyndman.com/papers/cv-wp.pdf) data** because contrary to other [cross-validation](https://en.wikipedia.org/wiki/Cross-validation_(statistics)) techniques, it does not break the sequential structure of the inputs. **Keep in mind**, however, that [time series cross-vaidation](https://thierrymoudiki.github.io/blog/2020/03/27/r/misc/crossval-2) will give different results. 
...
In the following example, it's worth mentioning that **only the 2 regularization parameters are calibrated**. Other hyperparameters such as the number of time series lags or the number of nodes in the hidden layer are set to their default values. Feel free try and share other experiments including them. 
...
1. **Univariate forecasting with `ridge2f` with external regressors:**
